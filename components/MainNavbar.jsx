...
export default function MainNavbar() {
    ...
    if (loading) {
        return <div>Loading....please wait for few seconds</div>
    }
    return (
        <>
            <div className="navbar bg-black p-5 fixed top-0 w-full z-10">
                <div className="flex-1">
                    <Link href='/user/homepage' className="btn  text-xl font-mono">Welcome {user.data.username} </Link>
                </div>
                <div className="flex-none">
                    <ul className="menu menu-horizontal px-1 text-lg font-bold">

                        <Link href='/user/Friends' className="p-3 rounded-full hover:bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500">Friends</Link>
